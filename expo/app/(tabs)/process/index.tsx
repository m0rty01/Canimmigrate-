  ...
  const parseAIResponse = useCallback((text: string): PathwayResult => {
    try {
      ...
      return {
        ...
        eligibility: ['Minimum CLB 7 in all language abilities', 'At least 1 year skilled work experience', 'Educational Credential Assessment (ECA)', 'Proof of settlement funds', 'Verify current requirements directly on canada.ca'],
        detailedProcess: ['Take IELTS/CELPIP language test', 'Get Educational Credential Assessment', 'Review official Express Entry instructions on canada.ca', 'Create Express Entry profile if appropriate', 'Monitor official government account/messages directly', 'Submit application through official government channels only', 'Provide biometrics if officially requested', 'Complete medical exam if officially requested'],
